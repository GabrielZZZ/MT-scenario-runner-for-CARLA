    ...
    def parse_scenario_configuration(scenario_name, config_file_name):
        ...
        for file_name in list_of_config_files:
            ...
            for scenario in tree.iter("scenario"):
                ...
                # Parse additional parameters from the XML file if they exist
                parameters = scenario.find('parameters')
                if parameters is not None:
                    new_config.first_actor_location = float(parameters.get('first_actor_location', '0.0'))
                    new_config.second_actor_location = float(parameters.get('second_actor_location', '0.0'))
                    new_config.first_actor_speed = float(parameters.get('first_actor_speed', '0.0'))
                    new_config.second_actor_speed = float(parameters.get('second_actor_speed', '0.0'))
                    
                    new_config.other_actor_max_brake = float(parameters.get('other_actor_max_brake', '0.0'))
                    new_config.other_actor_stop_in_front_intersection = float(parameters.get('other_actor_stop_in_front_intersection', '0.0'))
                    new_config.first_actor_type = parameters.get('first_actor_type', 'vehicle.tesla.model3')
                    new_config.second_actor_type = parameters.get('second_actor_type', 'vehicle.tesla.model3')
