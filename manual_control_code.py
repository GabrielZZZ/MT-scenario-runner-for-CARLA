...
current_duration_straight = None

current_command_start_time_steer = None
current_command_steer = None
current_duration_steer = None
...
class KeyboardControl(object):
    ...
    def parse_events(self, client, world, clock):
        ...
        if not self._autopilot_enabled:
            ...
            self._auto_control_vehicle(clock.get_time())
    ...
    def _auto_control_vehicle(self, milliseconds):
        global current_command_start_time_straight, current_command_straight, current_duration_straight
        global current_command_start_time_steer, current_command_steer, current_duration_steer
        

        if current_command_straight is None and not command_queue_straight.empty():
            # Get the next command from the queue
            current_command_straight, current_duration_straight = command_queue_straight.get()
            current_command_start_time_straight = time.time()
            
        if current_command_steer is None and not command_queue_steer.empty():
            # Get the next command from the queue
            current_command_steer, current_duration_steer = command_queue_steer.get()
            current_command_start_time_steer = time.time()
            
        # If there is a current command, execute it
        if current_command_straight is not None:
            # Execute the command
        ...
        if current_command_steer is not None:
            ...
            if current_command_steer == 'a':
                if self._steer_cache > 0:
                    self._steer_cache = 0
                else:
                    self._steer_cache -= steer_increment
            elif current_command_steer == 'd':
                if self._steer_cache < 0:
                    self._steer_cache = 0
                else:
                    self._steer_cache += steer_increment
            ...
            # If the command's duration has passed, clear the current command
            if time.time() - current_command_start_time_steer > current_duration_steer:
                current_command_steer = None
                self._control.steer = 0.0
                self._control.brake = 0.0

    def _parse_vehicle_keys(self, keys, milliseconds):
        # print("parse vehicle keys")
